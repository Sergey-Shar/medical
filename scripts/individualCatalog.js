...
const createReangeInput = () => {
	...
	const inputRangeFromMaxValue =
		document.querySelector('.range-input__from').max;
	...
	const getValueRangeFrom = () => {
		if (
			parseInt(inputRangeTo.value) - parseInt(inputRangeFrom.value) <=
			MIN_GAP
		) {
			inputRangeFrom.value = parseInt(inputRangeTo.value) - MIN_GAP;
		}
	...
	const getValueRangeTo = () => {
		if (
			...
			inputRangeTo.value = parseInt(inputRangeFrom.value) + MIN_GAP;
		}
...
const createReangeInputModal = () => {
	const MIN_GAP = 7;

	const inputRangeFrom = document.querySelector('.range-input__from-modal');
	const inputRangeTo = document.querySelector('.range-input__to-modal');

	const valueRangeFrom = document.querySelector(
		'.range-input__value-from-modal'
	);
	const valueRangeTo = document.querySelector('.range-input__value-to-modal');

	const rangeTrack = document.querySelector('.range-input__track-modal');

	const inputRangeFromMaxValue = document.querySelector(
		'.range-input__from-modal'
	).max;
...
const swiperCardDescription = document.querySelectorAll(
	'.swiper-card__description-block'
);
